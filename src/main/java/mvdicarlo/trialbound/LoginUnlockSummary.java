...
public class LoginUnlockSummary implements GroupStateListener {
    ...
    public LoginUnlockSummary(Client client, ConfigManager configManager, SessionState sessionState,
            GroupStateService groupState, TrialboundConfig config, TrialboundChat chat) {
        this.client = client;
    ...
    private long readWatermark() {
        String value = configManager.getRSProfileConfiguration(TrialboundPlugin.CONFIG_GROUP, KEY_LAST_SEEN);
    ...
    private void writeWatermark(long millis) {
        configManager.setRSProfileConfiguration(TrialboundPlugin.CONFIG_GROUP, KEY_LAST_SEEN, millis);
    }
}
